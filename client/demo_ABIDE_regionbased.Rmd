...
## Load the demographics and total brain measurements
...
The sample should be a subset of the available data (rows) in the data.frame 'demographics' that must:
...
1) Aggregate the native space data by atlas region from the raw per-vertex data files and parcellations. This takes quite a bit of time due to data loading for a large data set (and slow hard disks/networks). We do no do this here. We still illustrate how it could be done:

```{r, eval = FALSE}
braindata = fsbrain::group.agg.atlas.native(subjects_dir, subjects_list, measure=measure, hemi=hemi, atlas=atlas);
```

2) Alternatively, one could load a CSV file produced by the FreeSurfer tool `aparcstats2table` for your descriptor (one file per hemi). Then you would only need to do the computation once (when running that command), so here in R all you need to do is to load a CSV file, which is very fast.
...
```{r rgl=TRUE, dev='png'}
if(do_plot) {
    fsbrain::vis.subject.morph.native(subjects_dir, subjects_list[1], measure = measure);
}
```
...
if(do_plot) {
    ...
    measure_to_plot = "cohens.f";           # Choose one. Can use "cohens.f", "etasq", or "power".
    predictors_to_plot = c("group", "age"); # Choose one or more from variable 'predictors'.
    fsbrain_plots = list();
    ...
    for(pred_idx in seq.int(length(predictors))) {
        ...
        if(!(predictor %in% predictors_to_plot)) {
            next; # skip others.
        }

        measure_values = slm_res[[measure_to_plot]][predictor, ];

        # Our region names do not match the ones from the aparc atlas due to the lh_ and rh_ prefixes,
        # so we split into one listper hemisphere based on the prefixes.
        ...
        cat(sprintf("Writing %s figure for predictor '%s' to file '%s' in directory '%s'.\n", measure_to_plot, predictor, output_img, getwd()));
        img = fsbrain::export(cm, output_img = output_img, colorbar_legend = sprintf("%s for %s ", measure_to_plot, predictor));
        fsbrain_plots[[predictor]] = img;
    }

    # A single effect size (or whatever) violin plot for all predictors.
    ggplot_instance = brainnet::effect_size_violin_plots(slm_res[[measure_to_plot]], plot_ylabel=measure_to_plot);
}
```

Show the plots in the notebook (if any):
